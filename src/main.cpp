...
int main() {
  ...
  for (int n = 3; n <= 10; ++n) {
    ...
    auto timeAllPerms = std::chrono::duration_cast<
        std::chrono::microseconds>(end - start).count();

    PMTree treePerm1(testInput);
    int totalPerms1 = static_cast<int>(allPermsResult.size());
    int randomNum1 = getRandomPermNumber(totalPerms1);
    start = std::chrono::high_resolution_clock::now();
    getPerm1(treePerm1, randomNum1);
    end = std::chrono::high_resolution_clock::now();
    auto timePerm1 = std::chrono::duration_cast<
        std::chrono::microseconds>(end - start).count();

    PMTree treePerm2(testInput);
    int totalPerms2 = static_cast<int>(allPermsResult.size());
    int randomNum2 = getRandomPermNumber(totalPerms2);
    start = std::chrono::high_resolution_clock::now();
    getPerm2(treePerm2, randomNum2);
    end = std::chrono::high_resolution_clock::now();
    auto timePerm2 = std::chrono::duration_cast<
        std::chrono::microseconds>(end - start).count();

    timingFile << n << "\t\t" << timeAllPerms << "\t\t"
               << timePerm1 << "\t\t" << timePerm2 << std::endl;

    std::cout << "OK (getAllPerms: " << timeAllPerms << " мкс)"
              << std::endl;
  }
  timingFile.close();
  std::cout << std::endl;
